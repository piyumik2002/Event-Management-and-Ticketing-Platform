...
const Home = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        //We make a request to the GET /api/events endpoint we created on the backend.
        const response = await axios.get('http://localhost:5000/api/events');
        setEvents(response.data); // The received events are put into the state.
        setLoading(false);
      } catch (err) {
        console.error(err); 
  ...
  //The beautiful logic that filters events according to what the user types and selects
  const filteredEvents = events.filter((event) => {
    //Checks if the keyword is in the Title or Description (safely done with event.title?.toLowerCase())
    const matchesSearch = (event.title?.toLowerCase() || '').includes(searchQuery.toLowerCase()) || 
                          (event.description?.toLowerCase() || '').includes(searchQuery.toLowerCase());
    
    // Checking if the category matches (event.category?.toLowerCase() was used to avoid crashes)
    const matchesCategory = selectedCategory === '' || (event.category && event.category.toLowerCase() === selectedCategory.toLowerCase());
    
    // Checking if the venue matches (safely done with event.venue?.toLowerCase())
  ...
  //The beautiful loading screen that appears when data is loading.
  if (loading) {
    return (
  ...
  return (
    <div className="bg-slate-950 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 mb-12 relative z-20">
        <div className="bg-slate-900 border border-slate-800 p-4 md:p-6 rounded-2xl shadow-2xl space-y-4 md:space-y-0 md:flex md:items-center md:gap-4">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        {/* 3. The section that is displayed if no events match after searching. */}
        {filteredEvents.length === 0 ? (
          <div className="text-center py-20 bg-slate-900/50 border border-slate-800 rounded-2xl">
            <p className="text-slate-400 text-lg">No events match your search criteria. 🔍</p>
          </div>
        ) : (
          // Grid layout showing filtered events
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredEvents.map((event) => (
              <div key={event._id} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:transform hover:scale-[1.02] transition-all duration-300 shadow-lg flex flex-col justify-between">
                <div>
                  {/*The organizer will take the actual image URL, or show a default fallback. */}
                  <img 
                    ...
                    onError={(e) => {
                      // Added security to show the default image without crashing if an incorrect image URL is entered.
